...
- **고객 태블릿**: `http://localhost:8080/kiosk` — 메뉴(카테고리 탭) → 장바구니 → 받는 방법 → 결제 → 이름 → 완료
...
카테고리(커피/논커피/아이스크림/디저트…)·메뉴·옵션(샷 추가/연하게)·배달 장소는 **스태프 화면 > 설정** 에서 관리한다
(품절/판매중, 가격, 선택지, 순서, 추가/삭제). 키오스크 순서 = 카테고리 순서 → 카테고리 안 메뉴 순서.
...
- 충전은 한 번에 **30,000원까지**. **20,000원 충전이면 무료 1잔, 30,000원이면 무료 2잔** 적립 (그 사이 금액은 1잔, 20,000원 미만은 없음).
  고객이 쿠폰 화면에서 "무료 1잔 쓰기"를 체크하면
  그 주문에서 가장 비싼 한 잔 값이 먼저 빠지고 나머지만 잔액에서 차감된다.
- 쿠폰 등록에는 **전화번호가 필수** (동명이인 구분, 잔액 문자, 환불을 쿠폰에 넣을 때 혼동 방지). 키오스크에서 쿠폰을 쓸 때는
  이름으로 찾고, 같은 이름이 둘 이상일 때만 뒤 4자리를 묻는다. 번호 없는 옛 쿠폰은 번호를 넣기 전까지 충전할 수 없다.
- 스태프 쿠폰 화면에서 최근 한 달 이력(충전/사용/환불/정정, 주문 번호)이 보인다.
- **수정 차액**: 금액이 줄면 돌려줄 돈(현금으로 주거나 쿠폰 잔액에 넣기; 쿠폰 주문은 잔액으로 자동 복귀). 금액이 늘면 더 받을 돈이 남고 스태프가 **현금 / 계좌이체 / 쿠폰에서 빼기** 중 고른다 (쿠폰 주문이라도 자동으로 더 빼지 않는다). 정산 전엔 완료할 수 없다.
...
- **수정 후 차액**: 주문 때 실제 받은 현금/이체를 기억해 두고, 수정으로 금액이 달라지면 카드에
  ...
  / "현금·계좌이체 N원 더 받기" 를 빨갛게 띄운다. 처리한 뒤 버튼을 누르면 사라진다.
- **취소** 도 같은 방식: 받은 돈이 있으면 "현금으로 돌려주고 취소 / 쿠폰에 넣고 취소" 를 고른다. 쿠폰으로 낸 몫은 자동 복원.
