...
export function OptionAdmin({ onToast, categories }: { onToast: (msg: string) => void; categories: string[] }) {
  ...
  const add = () => run(async () => {
    await api.createOption({ name: name.trim(), price: parseMoney(price), category: category.trim(), group: group.trim() || null, available: true })
    setName('')
    setPrice('')
    setGroup('')
  }, `${name.trim()} 옵션 추가됨`)
  ...
  const saveEdit = (o: AdminOption) => run(async () => {
    if (!editing) return
    await api.updateOption(o.id, { name: editing.name.trim(), price: parseMoney(editing.price), category: o.category,
    ...
    <div className="stack">
      <div className="card">
        ...
        <div className="row">
          ...
          <input className="text-input" style={{ width: 110 }} inputMode="numeric" placeholder="+원" value={price} maxLength={9}
            onChange={(e) => setPrice(formatMoneyInput(e.target.value))} aria-label="추가 금액" />
        </div>
        ...
        {price !== '' && !isPrice(parseMoney(price)) && <div className="error" style={{ fontSize: 13 }}>금액은 100원 단위, {won(RULES.priceMax)}까지입니다.</div>}
        <button className="btn primary" disabled={busy || !name.trim() || !isPrice(parseMoney(price))} onClick={() => void add()}>추가</button>
      </div>
        ...
        <div key={o.id} className={'card admin-item' + (o.available ? '' : ' off')}>
            ...
            {editing?.id === o.id ? (
              <div className="stack grow" style={{ gap: 6 }}>
                <div className="row">
                  ...
                  <input className="text-input" style={{ width: 110, minHeight: 44, fontSize: 16 }} inputMode="numeric" value={editing.price} maxLength={9}
                    onChange={(e) => setEditing({ ...editing, price: formatMoneyInput(e.target.value) })} aria-label="옵션 금액" placeholder="+원" />
                </div>
                <div className="row">
                  ...
                  <button className="btn primary" style={{ minHeight: 44, fontSize: 14 }} disabled={busy || !editing.name.trim() || !isPrice(parseMoney(editing.price))} onClick={() => void saveEdit(o)}>저장</button>
          ...
          <div className="row admin-actions">
            ...
            <button className="btn" disabled={busy || editing?.id === o.id}
              onClick={() => setEditing({ id: o.id, name: o.name, price: o.price ? o.price.toLocaleString('ko-KR') : '', group: o.group ?? '' })}>수정</button>
