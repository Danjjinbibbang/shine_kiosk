...
CREATE TABLE IF NOT EXISTS coupon (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    name        TEXT    NOT NULL,
    phone       TEXT,                          -- 전체 번호 (숫자만). 잔액 문자 발송용, 뒤 4자리는 동명이인 구분용. 없으면 NULL
...
CREATE TABLE IF NOT EXISTS orders (
    ...
    cash_given      INTEGER,                   -- 손님이 낸 현금. 현금 안 냈으면 NULL
    change_credited INTEGER NOT NULL DEFAULT 0, -- 거스름돈 중 쿠폰에 넣은 금액
    change_paid     INTEGER NOT NULL DEFAULT 0, -- 완료하면서 손에 쥐어 준 거스름돈. 남은 거스름돈 = cash_given - cash_amount - change_credited - change_paid
    refund_cash     INTEGER NOT NULL DEFAULT 0, -- 수정/취소로 현금으로 돌려준 돈
    refund_transfer INTEGER NOT NULL DEFAULT 0, -- 수정/취소로 계좌이체로 돌려준 돈 (쿠폰에 넣은 건 coupon_tx 에)
